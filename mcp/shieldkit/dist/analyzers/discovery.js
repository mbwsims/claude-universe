...
export async function discoverRouteFiles(cwd) {
    const patterns = [
...
export async function discoverEnvFiles(cwd) {
...
export async function discoverDbFiles(cwd) {
    const allSource = await discoverSourceFiles(cwd);
    const dbFiles = [];
    for (const file of allSource) {
        try {
            const content = await readFile(join(cwd, file), 'utf-8');
            const hasDbAccess = /\b(prisma|knex|sequelize|typeorm|mongoose|mongodb|pg\.query|mysql|sqlite|drizzle)\b/i.test(content) ||
...
export async function checkGitignore(cwd, filePath) {
    try {
        ...
        for (const pattern of lines) {
            ...
            if (pattern.endsWith('/') && filePath.startsWith(pattern.slice(0, -1)))
                return true;
        }
    }
    catch {
