...
function computeMonthsDiff(sinceDate, untilDate) {
    ...
    const months = (until.getFullYear() - since.getFullYear()) * 12 +
...
async function getCommitMessages(since, cwd, file) {
...
async function getCommitCount(since, cwd, file) {
...
async function getAuthors(since, cwd, file) {
...
async function getMostChangedFiles(since, cwd) {
...
async function getSizeOverTime(since, file, cwd) {
    ...
    const selected = entries.length <= 10
        ? entries
        : entries.filter((_, i) => i % Math.ceil(entries.length / 10) === 0 || i === entries.length - 1);
    const snapshots = [];
    for (const entry of selected) {
        try {
            const { stdout: fileContent } = await execFile('git', ['show', `${entry.hash}:${file}`], { cwd, maxBuffer: 10 * 1024 * 1024 });
...
export async function analyzeHistory(args, cwd) {
    const since = args.since ?? '6 months ago';
    // Resolve the "since" date for the output period
    const sinceOutput = await gitRun(['log', '--format=%aI', `--since=${since}`, '--reverse', '-1'], cwd);
    const sinceDate = sinceOutput.trim().split('T')[0] || new Date(Date.now() - 6 * 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
