...
export interface MockHealthResult {
  ...
  internal: number;
  setupPercent: number;
  mocks: Array<{ line: number; path: string; type: 'boundary' | 'internal' }>;
}
...
export function analyzeMockHealth(content: string): MockHealthResult {
  const lines = content.split('\n');
  const mocks: MockHealthResult['mocks'] = [];
