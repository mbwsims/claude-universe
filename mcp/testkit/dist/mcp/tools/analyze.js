...
import { scoreAssertionDepth, scoreErrorTesting, scoreMockHealth, scoreSpecClarity, computeOverallGrade, valueToGrade, GRADE_VALUES, } from '../../analyzers/scoring.js';
import { discoverTestFiles, detectFramework, inferSourcePath } from '../../analyzers/discovery.js';
async function analyzeFile(testPath, cwd, framework) {
...
export async function analyzeTool(args, cwd) {
    ...
    const files = await Promise.all(testPaths.map(p => analyzeFile(p, cwd, framework)));
