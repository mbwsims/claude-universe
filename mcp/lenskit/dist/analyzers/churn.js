 ...
 */
export async function analyzeChurn(filePath, cwd) {
    let changes = 0;
    let authors = 0;
    try {
        const { stdout: logOutput } = await execFile('git', ['log', '--oneline', '--since=6 months ago', '--', filePath], { cwd });
    ...
    try {
        const { stdout: authorOutput } = await execFile('git', ['log', '--format=%an', '--since=6 months ago', '--', filePath], { cwd });
 ...
 */
export async function batchAnalyzeChurn(cwd) {
    const results = new Map();
    // One git log for all file change counts
    let changesByFile = new Map();
    try {
        const { stdout } = await execFile('git', ['log', '--format=format:', '--name-only', '--since=6 months ago'], { cwd, maxBuffer: 50 * 1024 * 1024 });
    ...
    let authorsByFile = new Map();
    try {
        const { stdout } = await execFile('git', ['log', '--format=COMMIT_SEP %an', '--name-only', '--since=6 months ago'], { cwd, maxBuffer: 50 * 1024 * 1024 });
        let currentAuthor = '';
        for (const line of stdout.split('\n')) {
            const trimmed = line.trim();
            if (trimmed === '')
